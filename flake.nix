  ...
  outputs = {
    ...
    flake-utils.lib.eachDefaultSystem (
      system: let
        ...
        pythonP2NWEnv =
          #pythonEnv from poetry2nix using wheels (aka admitting defeat)
          pkgs.poetry2nix.mkPoetryEnv {
            projectDir = ./pyenvs/default;
            preferWheels = true;
            overrides = p2n-overrides {
              sphinxcontrib-jquery = ["sphinx"];
              nvidia-cusparse-cu12.propagatedBuildInputs = [pkgs.cudaPackages.libnvjitlink];
              nvidia-cusolver-cu12.propagatedBuildInputs = with pkgs.cudaPackages; [
                libnvjitlink
                libcusparse
                libcublas
          ...
          with pkgs; (python3.withPackages (
            ps:
              with ps; [
                ...
                pymatgen
                scipy
                # FIXME: tensorflow has a hack which installs a custom tensorboard
                # this conflicts with torch dependency
                #(tensorflow.override ({tensorboardSupport = false;})) #big compile
                torch
              ]
          ));
      ...
      in {
        devShell = pkgs.mkShell {
          buildInputs = with pkgs; [
            #pythonNPEnv  # missing packages
            pythonP2NEnv # compiles many things, needs overrides
            #pythonP2NWEnv # using wheels
          ];
        };
      }
    );
